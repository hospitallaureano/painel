    ...
    <div class="columns is-gapless">
      <div class="column is-multiline featured-column">
        ...
        <footer class="column" :style="{ 'background-color': config.footerBgColor, 'color': config.footerFontColor }">
          <img src="static/images/laureano-image.jpg">
        </footer>
      </div>
  ...
  export default {
    ...
    methods: {
      playAudio () {
        ...
        const pass = ['Senha']
        const clientName = [this.$store.getters.message.$data.nomeCliente]
        const talking = pass.concat(this.$store.getters.message.falar).concat(clientName)
        talking.forEach(word => {
          if (typeof word === 'number' || (word && word.length > 0)) {
            setTimeout(() => {
              speak.speechAll([word], lang).then(() => {
                log('Testing end')
              }, (e) => {
                log('Testing error', e)
              })
